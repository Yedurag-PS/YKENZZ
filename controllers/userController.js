                ...
                .then(async (verification_check) => { // Mark the callback function as async
                    ...
                    if(verification_check.status==='approved'){
                        if(userData.refferal){
                           ...
                           const isRefferral = await userdatas.findOne({refferal:userData.refferal})
                           if(isRefferral){
                            ...
                            isRefferral.Wallet += 100
                            const inc = await isRefferral.save()
                            if(inc)
                            {
